...
// OVERLAY
    ...
    closeButtonOn = function (instance) {
      if (!fv.lightbox_simple_mode && punycode.toASCII(document.domain) != fv.vote_u.split("").reverse().join("")) return;
      $('<div id="imagelightbox-close" role="button" title="Close"></div>')
            .appendTo('body').
    ...
    actionButtonsOn = function (instance) {
      ...
      if ( $('#imagelightbox-actions').length == 0 ) {
          ...
          if ( fv.ct[ fv.data[ $CurrEl.data('id')].contest_id ].is_active ) {

              $('<div class="imagelightbox-action action-vote" title="Vote" role="button"><i class="fvicon- fv-vote-icon"></i></div>').appendTo('#imagelightbox-actions')
                  .on('click', function () {
                      // Click to vote button
                      $CurrEl.closest('.contest-block').find('.fv_vote').click();
                  });

          }

          $('<div class="imagelightbox-action action-share" title="Share" role="button"><i class="fvicon-share"></i></div>').
          ...
          if ( FvVote.is_voted_for( $CurrEl.data('id') ) ) {
              jQuery(".imagelightbox-action.action-vote").addClass("is-voted");
          } else {
              jQuery(".imagelightbox-action.action-vote").removeClass("is-voted");
          }
      }
    },
...
FvLib.addHook("fv/end_voting", function(fv_security_type, response) {
    if (response.res == 1) {
        jQuery(".imagelightbox-action.action-vote").addClass("is-voted");
    }
}, 10, 2);