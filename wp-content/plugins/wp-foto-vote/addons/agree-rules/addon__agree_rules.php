...
function FvAddon_UploadAgreeRulesInit2(){
	...
	class FvAddon_UploadAgreeRules extends FvAddonBase {
		...
		public function init() {
			...
			if ( !is_admin() ) {
				...
				if ( $this->_get_opt('enabled') ) {
				    wp_add_notice('Addon "Upload Agree Rules" is deprecated and soon will be removed. Please use "Rules Checkbox" instead.
				     <a href="https://yadi.sk/i/jroeMghRxEg9D" target="_blank">https://yadi.sk/i/jroeMghRxEg9D</a>', 'warning');
					add_filter( 'fv_upload_form_rules_filer', array($this, 'upload_form_rules_hook_agree'), 10, 2 );
					add_action( 'wp_footer', array($this, 'footer_js'), 49 );
				}
			} else {
                if ( $this->_get_opt('enabled') ) {
                    wp_add_notice('Addon "Upload: agree with rules" is deprecated and soon will be removed. Please use "Rules Checkbox" instead.
				     <a href="https://yadi.sk/i/jroeMghRxEg9D" target="_blank">https://yadi.sk/i/jroeMghRxEg9D</a>', 'warning');
                }
            }
		...
		{
			if ( !wp_script_is('fv_upload_js', 'enqueued') ) {
				return;
			}
			echo '<script> FvLib.addHook("fv/upload_before_start", function (form) {
					if ( !form.querySelector(".fv_rules").checked ) { alert("' . sanitize_text_field($this->get_maybe_translated_form_string('error')) . '");return false; } return true;
				}, 10, 1); </script>';
		}
