FvLib.addHook('doc_ready', function() {
    if ( jQuery('div.fv-countdown-default').hasClass('countdownRunned') ) {
        return;
    }
    jQuery('div.fv-countdown-default').addClass('countdownRunned');

	if (document.createElement('canvas').getContext) {
		var image = jQuery('<img />').on("load", function() {
			var image = this,
				offset_left = 5,
				offset_number = 4;
			...
			var seconds_cnt = jQuery('div.fv-countdown-default .bl-inner:last'),
				minutes_cnt = jQuery('div.fv-countdown-default .bl-inner:eq(2)'),
				hourses_cnt = jQuery('div.fv-countdown-default .bl-inner:eq(1)'),
				days_cnt 	= jQuery('div.fv-countdown-default .bl-inner:eq(0)');
	
			seconds(parseInt(jQuery('span', seconds_cnt).text(), 10), jQuery('canvas', seconds_cnt)[0]);
		});
		image.attr('src',  jQuery('div.fv-countdown-default').data('image') );
	} else {
