...
class Xoo_El_Form_Handler{
	...
	public static function process_login(){
		...
		try{
			...
			self::verify_captcha('login'); // Verify captcha
	...
	public static function process_registration() {
		...
		if ( isset($_POST['_xoo_el_form']) &&  $_POST['_xoo_el_form'] === 'register' ) {
			...
			try {
				...
				self::verify_captcha('register'); // Verify captcha
				...
				$reg_admin_fields = xoo_el()->fields->get_fields_data();
				...
				if( class_exists('Xoo_Uv_Core') && !xoo_uv_core()->is_user_active( $user->ID )){
	...
	public static function verify_captcha($form_type){

		$av_options = get_option( 'xoo-el-advanced-options' );

		//Return if recaptcha is already verified / recpatcha not present
		if( isset( $_SESSION['xoo_el_captcha_verified'] ) || ( $form_type && isset( $av_options['r-en-recaptcha'] ) &&!in_array( $form_type, $av_options['r-en-recaptcha'] ) ) ){
			return;
		}

		if( !isset( $_POST['g-recaptcha-response'] ) || empty( $_POST['g-recaptcha-response'] ) ){
			$notice = __( 'Please verify you are a human.', 'easy-login-woocommerce' );
		}
		else{
			$secret_key = $av_options['r-recaptcha-secretkey'] ;
			$recaptcha 	= sanitize_text_field( $_POST['g-recaptcha-response'] );
			$response 	= json_decode( file_get_contents( "https://www.google.com/recaptcha/api/siteverify?secret=".$secret_key."&response=".$recaptcha."&remoteip=".$_SERVER['REMOTE_ADDR'] ), true );
			if( $response['success'] == '1' ){
				$_SESSION['xoo_el_captcha_verified'] = "yes"; //Store it in a session.
				return true;
			}
			else{
				$notice = __( "Couldn't verify recaptcha. Please try again.", "easy-login-woocommerce" );
			}
		}

		if( isset( $notice ) ){
			throw new Xoo_El_Exception( $notice );
		}

	}


	public static function get_error_data($e){
